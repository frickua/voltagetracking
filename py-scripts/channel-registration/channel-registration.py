...
def send_welcome_msg(user_id):
    ...
    if response and len(response.data) > 0:
        ...
        if  data['auth_key'] and data['tg_user_id']:
            send_tg_msg(response.data[0]['tg_user_id'],
        f"""
...
{"\n".join(f"\t- {row['chat_name']}: {row['auth_key']}" for row in response.data)}
...
Тіло запиту(body):
<pre><code>voltage,key={response.data[0]['auth_key']},phase=1 value=242.7</code></pre>\n\n
...
for update in resp["result"]:
    ...
    if 'message' in update:
        ...
        if 'text' in msg:
            msg_txt = msg['text']
            if msg_txt == '/start':
                if not send_welcome_msg(msg['from']['id']):
